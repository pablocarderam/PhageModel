...
fitLogisticEquation = function(indepVar,depVar1,depVar2,depVar1_Err,depVar2_Err,nameDepVar1,nameDepVar2,est_K,title,title_x,title_y,col1,col2) {
  ...
  graph = ggplot(data=df,aes(x=time,y=value))+
    geom_point(aes(x=time,y=value,shape=variable),size=2)+
    geom_errorbar(aes(ymin=value-Err,ymax=value+Err),width=0.6,size=0.75,alpha=0.75)+
    geom_line(data=fit,aes(x=x,y=y1), size=1,linetype=1,alpha=0.5)+
    geom_line(data=fit,aes(x=x,y=y2), size=1,linetype=3,alpha=0.5)+
    scale_shape_discrete("")+ # "" Removes legend title
...
ggsave(filename="LogisticFit_BW.png",path="plt/",plot=gFit,width = 5, height = 5, units = c("in"), dpi = 300)
